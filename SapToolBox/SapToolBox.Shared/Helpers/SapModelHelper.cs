...
namespace SapToolBox.Shared.Helpers {
    ...
    #region 字段属性
        ...
        public string DesignCode {
            get {
                _sapModel.DesignSteel.GetCode(ref _designCode);
                return _designCode;
            }
            set {
                _sapModel.DesignSteel.SetCode(value);
    ...
    #region 构造函数
        ...
        public SapModelHelper(ref cSapModel sapModel) {
            _sapModel = sapModel;

            _sapModel.GroupDef.GetNameList(ref _groupNum, ref _groupList);

            _sapModel.FrameObj.GetNameList(ref _frameNum, ref _frameNameList);
    ...
    #region 方法
        ...
        public void SetOverwrite(string groupName, int propIndex, double propValue) {
            switch (DesignCode) {
                case "Chinese 2018":
                    _sapModel.DesignSteel.Chinese_2018.SetOverwrite(groupName, propIndex, propValue, eItemType.Group);
                    break;
                case "Chinese 2010":
                    _sapModel.DesignSteel.Chinese_2010.SetOverwrite(groupName, propIndex, propValue, eItemType.Group);
                    break;
            }
        }

        public void SetOverwrite(int propIndex, double propValue) {
            switch (DesignCode) {
                case "Chinese 2018":
                    _sapModel.DesignSteel.Chinese_2018.SetOverwrite(null, propIndex, propValue,
                                                                    eItemType.SelectedObjects);
                    break;
                case "Chinese 2010":
                    _sapModel.DesignSteel.Chinese_2010.SetOverwrite(null, propIndex, propValue,
                                                                    eItemType.SelectedObjects);
                    break;
            }
        }
            ...
            switch (DesignCode) {
                case "Chinese 2018":
                    canModifyDic = new Dictionary<int, string> {
                                                                   { 16, "总荷载挠度限值" }, { 24, "净/毛面积比" },
                                                                   { 26, "无支撑长度系数(主)" }, { 27, "无支撑长度系数(次)" },
                                                                   { 28, "有效长度系数(主)" }, { 29, "有效长度系数(次)" },
                                                                   { 36, "梁的整体稳定性系数(主)" }, { 37, "梁的整体稳定性系数(次)" },
                                                                   { 38, "塑性发展系数(主)" }, { 39, "塑性发展系数(次)" },
                                                                   { 40, "截面影响系数" }, { 41, "强柱系数" }, { 44, "屈服应力" },
                                                                   { 45, "抗弯强度设计值" }, { 46, "抗剪强度设计值" },
                                                                   { 48, "应力比限值" }, { 50, "受压长细比限值" }, { 51, "受拉长细比限值" }
                                                               };
                    OverWriteObjs = [];
                    for (var i = 0; i < 52; i++) {
                        double value = 0;
                        var    flag  = false;
                        var    obj   = new OverWriteObj { Index = i + 1 };
                        if (canModifyDic.ContainsKey(obj.Index)) {
                            _sapModel.DesignSteel.Chinese_2018.GetOverwrite(FrameNameList.First(), obj.Index, ref value,
                                                                            ref flag);
                            obj.Value       = value;
                ...
                case "Chinese 2010":
                    ...
                    for (var i = 0; i < 51; i++) {
                        ...
                        var    obj   = new OverWriteObj { Index = i + 1 };
                        if (canModifyDic.ContainsKey(obj.Index)) {
                            _sapModel.DesignSteel.Chinese_2010.GetOverwrite(FrameNameList.First(), obj.Index, ref value,
                                                                            ref flag);
                            obj.Value       = value;
                            obj.DisplayName = canModifyDic[obj.Index];
                            obj.IsDefault   = flag;
                    ...
                    _overwriteDictionary = new Dictionary<int, PropertyObj> {
                                                                                {
                                                                                    ...
                                                                                    new PropertyObj {
                                                                                        ChineseName = "框架类型",
                                                                                        EnglishName = "Framing type"
                                                                                    }
                                                                                }, {
                                                                                    2,
                                                                                    new PropertyObj {
                                                                                        ChineseName = "构件类型",
                                                                                        EnglishName = "Element type"
                                                                                    }
                                                                                }, {
                                                                                    3,
                                                                                    new PropertyObj {
